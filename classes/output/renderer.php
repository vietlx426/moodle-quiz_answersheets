...
class renderer extends plugin_renderer_base {
     ...
     */
    public function render_question_attempt_content(
        ...
        foreach ($slots as $slot) {
            ...
            if (
                $currentquestion->get_type_name() === 'essay' &&
                has_capability('quiz/answersheets:submitresponses', $context) &&
                !empty($currentquestion->attachments) &&
                $attempt->state == 'finished' &&
                ($quiz->timeclose != 0 && $quiz->timeclose < time())
            ) {
                $essay = $qoutput->question($qa, $behaviouroutput, $qtoutput, $cloneddisplayoptions, $questionnumber);

                $editlink = html_writer::link(
                    new moodle_url(
                        '/mod/quiz/report/answersheets/edit_attachment.php',
                        [
                    ...
                    get_string('essay_edit_attachment', 'quiz_answersheets'),
                    ['class' => 'edit-attachment-link ml-5']
                );
                $essay = preg_replace(
                    '/(<div\s+class="info">.*?)(<\/div>)/s',
                    '$1' . $editlink . '$2',
                    $essay,
                    1
                );
     ...
     * @return string HTML string
     */
    public function render_question_attempt_form(
        quiz_attempt $attemptobj,
        report_display_options $reportoptions,
        string $redirect = ''
    ): string {
        ...
        $output .= html_writer::tag(
            'button',
            get_string(
                'submit_student_responses_on_behalf',
                'quiz_answersheets',
    ...
    public function render_print_button(quiz_attempt $attemptobj, bool $isrightanswer): string {
        ...
        return html_writer::tag(
            'button',
            get_string('print', 'quiz_answersheets'),
     ...
     * @return string HTML string
     */
    public function render_attempt_sheet(
        array $sumdata,
        quiz_attempt $attemptobj,
        string $sheettype,
        report_display_options $reportoptions
        ...
        $summaryinfo = attempt_summary_information::create_from_legacy_array($sumdata);
        $templatecontext = [
            'questionattemptheader' => utils::get_attempt_sheet_print_header($attemptobj, $sheettype, $reportoptions),
            'questionattemptsumtable' => $quizrenderer->review_attempt_summary($summaryinfo, 0),
    ...
    public function bulk_download_link(report_display_options $options): string {
        ...
        return html_writer::div(
            html_writer::link($bulkurl, get_string('bulkdownloadlink', 'quiz_answersheets'))
        );
    }
     ...
     * @return string HTML.
     */
    public function bulk_download_instructions(
        ...
        $output .= $this->output->notification(
            get_string('bulkinstructionswarning', 'quiz_answersheets'),
            \core\output\notification::NOTIFY_WARNING
        );
        $output .= format_text(
            get_string('bulkinstructions', 'quiz_answersheets', $a),
            FORMAT_MARKDOWN,
            ['context' => $context]
        );
        return $output;
